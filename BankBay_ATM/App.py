    ...
    def __mainwindow(self,cardNumber):
        ...
        def withdraw(cardNumber):
            def withdrawl_process(cardNumber,amount):
                ...
                sql = "SELECT * FROM atm_accountdetails WHERE account_number='{}';".format(result[1])
                mycursor.execute(sql)
                result1 = mycursor.fetchone()
                balance=int(result1[4])
                if(amount >= 100):
                    if(amount <=40000):
                        if(amount%100==0):
                            if(amount>balance):
                                ...
                                try:
                                    sql = "UPDATE atm_accountdetails SET balance = balance - {} WHERE account_number='{}';".format(amount,result[1])
        ...
        def info(cardNumber):
            ...
            sql="SELECT * FROM  atm_personaldetails WHERE account_number='{}';".format(result[1])
            mycursor.execute(sql)
            ...
            self.name_label=Label(self, text=result2[1], font=("Consolas",30,"bold"),bg='navy', fg='yellow')
            ...
            self.savingsaccountbalance_label=Label(self, text="₹ {0:.2f}".format(result1[4]), font=("OCR A Extended",27,"bold"),bg='navy', fg='yellow')
        ...
        self.paymentsummary_btn= Button(self,text="Mini Statement",font=("Segoe UI Variable Text Light",20,"bold"),height=2,width=23,fg="white",bd=0,bg="SlateBlue2",relief=FLAT,state=DISABLED)
